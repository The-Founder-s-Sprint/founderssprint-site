...
(function() {
  ...
  function loadSponsors() {
    ...
    var _sc = ('AbortController' in window) ? new AbortController() : null;
    var _st = setTimeout(function(){ if(_sc){ try{ _sc.abort(); }catch(e){} } }, 8000); // stall ⇒ abort ⇒ .catch hides the strip
    fetch(url, {
      headers: {
      ...
      signal: _sc ? _sc.signal : undefined
    })
    .then(function(res) { clearTimeout(_st); return res.ok ? res.json() : []; })
