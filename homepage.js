...
(() => {
  ...
  (function(){
    ...
    function twFetch(path,o){
      // Each attempt is aborted after 8s so a silent mobile stall can't leave the section pending forever.
      function once(u){ var c=('AbortController' in window)?new AbortController():null; var t=setTimeout(function(){if(c){try{c.abort();}catch(e){}}},8000); return fetch(u,Object.assign({},o||{},{signal:c?c.signal:undefined})).then(function(r){clearTimeout(t);return r;},function(e){clearTimeout(t);throw e;}); }
      return once('/db'+path).then(function(r){ var ct=r.headers.get('content-type')||''; if(r.ok&&ct.indexOf('json')!==-1) return r; throw 0; }).catch(function(){ return once(SB_URL+path); }); }
